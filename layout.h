...
LAY_EXPORT uint32_t lay_get_contain(lay_context *ctx, lay_id item);
...
// Flex direction
typedef enum lay_flex_direction {
    LAY_ROW = 0x000,           // left to right (default)
    LAY_COLUMN = 0x001,        // top to bottom
    LAY_ROW_REVERSE = 0x002,   // right to left
    LAY_COLUMN_REVERSE = 0x003 // bottom to top
} lay_flex_direction;

// Layout model
typedef enum lay_layout_model {
    LAY_MODEL_LAYOUT = 0x000,  // free layout (absolute positioning)
    LAY_MODEL_FLEX = 0x004     // flex model
} lay_layout_model;

// Flex wrap
typedef enum lay_flex_wrap {
    LAY_WRAP_NO = 0x000,       // single-line, no wrap
    LAY_WRAP = 0x008,          // multi-line, wrap left to right
    LAY_WRAP_REVERSE = 0x010   // multi-line, wrap right to left
} lay_flex_wrap;

// Justify content
typedef enum lay_justify_content {
    LAY_JUSTIFY_START = 0x000,      // pack items at start (left for row, top for column)
    LAY_JUSTIFY_CENTER = 0x020,     // pack items at center
    LAY_JUSTIFY_END = 0x040,        // pack items at end (right for row, bottom for column)
    LAY_JUSTIFY_SPACE_BETWEEN = 0x060,  // space items evenly, first at start, last at end
    LAY_JUSTIFY_SPACE_AROUND = 0x080,   // space items evenly with equal space around
    LAY_JUSTIFY_SPACE_EVENLY = 0x0A0    // space items evenly with equal space between all
} lay_justify_content;

// Align items (for cross-axis alignment)
typedef enum lay_align_items {
    LAY_ALIGN_STRETCH = 0x000,     // stretch to fill container (default)
    LAY_ALIGN_START = 0x100,       // align to start (left for column, top for row)
    LAY_ALIGN_CENTER = 0x200,      // align to center
    LAY_ALIGN_END = 0x300,         // align to end (right for column, bottom for row)
    LAY_ALIGN_BASELINE = 0x400     // align along text baseline (if applicable)
} lay_align_items;

// Align content (for multi-line flex containers)
typedef enum lay_align_content {
    LAY_ALIGN_CONTENT_START = 0x000,      // pack lines at start
    LAY_ALIGN_CONTENT_CENTER = 0x800,     // pack lines at center
    LAY_ALIGN_CONTENT_END = 0x1000,       // pack lines at end
    LAY_ALIGN_CONTENT_SPACE_BETWEEN = 0x1800, // space lines evenly, first at start, last at end
    LAY_ALIGN_CONTENT_SPACE_AROUND = 0x2000,  // space lines evenly with equal space around
    LAY_ALIGN_CONTENT_STRETCH = 0x2800        // stretch lines to fill container (default)
} lay_align_content;

// Bit masks for extracting each property
#define LAY_FLEX_DIRECTION_MASK    0x0003  // bits 0-1
#define LAY_LAYOUT_MODEL_MASK      0x0004  // bit 2
#define LAY_FLEX_WRAP_MASK         0x0018  // bits 3-4
#define LAY_JUSTIFY_CONTENT_MASK   0x00E0  // bits 5-7
#define LAY_ALIGN_ITEMS_MASK       0x0700  // bits 8-10
#define LAY_ALIGN_CONTENT_MASK     0x3800  // bits 11-13

// Helper functions to extract individual properties from flags
LAY_STATIC_INLINE lay_flex_direction lay_get_flex_direction(uint32_t flags) {
    return (lay_flex_direction)(flags & LAY_FLEX_DIRECTION_MASK);
}

LAY_STATIC_INLINE lay_layout_model lay_get_layout_model(uint32_t flags) {
    return (lay_layout_model)(flags & LAY_LAYOUT_MODEL_MASK);
}

LAY_STATIC_INLINE lay_flex_wrap lay_get_flex_wrap(uint32_t flags) {
    return (lay_flex_wrap)(flags & LAY_FLEX_WRAP_MASK);
}

LAY_STATIC_INLINE lay_justify_content lay_get_justify_content(uint32_t flags) {
    return (lay_justify_content)(flags & LAY_JUSTIFY_CONTENT_MASK);
}

LAY_STATIC_INLINE lay_align_items lay_get_align_items(uint32_t flags) {
    return (lay_align_items)(flags & LAY_ALIGN_ITEMS_MASK);
}

LAY_STATIC_INLINE lay_align_content lay_get_align_content(uint32_t flags) {
    return (lay_align_content)(flags & LAY_ALIGN_CONTENT_MASK);
}

// Helper function to combine all properties into one flags value
LAY_STATIC_INLINE uint32_t lay_make_box_flags(
    ...
    return (uint32_t)direction |
           ...
           (uint32_t)align_content;
}

// Helper function to set container with individual properties
